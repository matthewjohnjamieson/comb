...
class CellController{
  ...
  eventClickedMouseOver(){
    if(mouseIsPressed && red( colorUnderMouse() /* get(mouseX, mouseY)*/ ) == red(this.cellView.mapColor) && (this.isClicked === false)){
      this.cellView.displayColor = 'BLACK';
      console.log(this.cellNumber);//print the current cell number
      this.cellModel.chord.play();
      this.isClicked = true;
    }else if(red( colorUnderMouse() /* get(mouseX, mouseY)*/ ) != red(this.cellView.mapColor) || (!mouseIsPressed && (this.isClicked === true))){
